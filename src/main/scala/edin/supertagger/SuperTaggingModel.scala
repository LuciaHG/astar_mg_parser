...
class SuperTaggingModel(
  ...
  def computeLoss(instance:IndexedInstance[TrainInst]) : Expression = {
    ...
    val sublosses = (tags zip logsoftmaxes).map{ case (t, lsf) =>
        -lsf(allS2I.t2i(t))
    }
    val loss = Expression.sum(sublosses)
    loss
  }
  ...
  override
  def filterTrainingData(trainData:Iterable[IndexedInstance[TrainInst]]) : Iterator[IndexedInstance[TrainInst]] = trainData.iterator
...
object SuperTaggingModel{
  ...
  def loadTokens(fn:String) : Iterable[List[String]] = {
    Source.fromFile(fn).getLines().map{ line =>
      line.split(" +").toList
    }.toList
  }
